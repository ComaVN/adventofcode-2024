...
class Region:
    ...
    def add_plot(self, plot):
        ...
        if (x - 1, y) not in self.plots:
            # new fence segment on the left
            self.fences += 1
            if (x - 1, y - 1) not in self.plots and (x, y - 1) in self.plots:
                # fence extends from the top
                self.fences -= 1
            if (x - 1, y + 1) not in self.plots and (x, y + 1) in self.plots:
                # fence extends from the bottom
                self.fences -= 1
        else:
            # no fence segment on the left anymore
            self.fences -= 1
            if (x - 1, y - 1) in self.plots and (x, y - 1) not in self.plots:
                # fence extended from the top
                self.fences += 1
            if (x - 1, y + 1) in self.plots and (x, y + 1) not in self.plots:
                # fence extended from the bottom
                self.fences += 1

        if (x + 1, y) not in self.plots:
            # new fence segment on the right
            self.fences += 1
            if (x + 1, y - 1) not in self.plots and (x, y - 1) in self.plots:
                # fence extends from the top
                self.fences -= 1
            ...
            # no fence segment on the right anymore
            self.fences -= 1
            if (x + 1, y - 1) in self.plots and (x, y - 1) not in self.plots:
                # fence extended from the top
                self.fences += 1
        ...
        if (x, y - 1) not in self.plots:
            # new fence segment on the top
            self.fences += 1
            if (x - 1, y - 1) not in self.plots and (x - 1, y) in self.plots:
                # fence extends from the left
                self.fences -= 1
            if (x + 1, y - 1) not in self.plots and (x + 1, y) in self.plots:
                # fence extends from the right
                self.fences -= 1
        else:
            # no fence segment on the top anymore
            self.fences -= 1
            if (x - 1, y - 1) in self.plots and (x - 1, y) not in self.plots:
                # fence extended from the left
                self.fences += 1
            if (x + 1, y - 1) in self.plots and (x + 1, y) not in self.plots:
                # fence extended from the right
                self.fences += 1

        if (x, y + 1) not in self.plots:
            # new fence segment on the bottom
            self.fences += 1
            if (x - 1, y + 1) not in self.plots and (x - 1, y) in self.plots:
                # fence extends from the left
                self.fences -= 1
            ...
            # no fence segment on the bottom anymore
            self.fences -= 1
            if (x - 1, y + 1) in self.plots and (x - 1, y) not in self.plots:
                # fence extended from the left
                self.fences += 1
...
class Map:
    ...
    def price(self):
        return sum(r.size() * r.fences for r in self.regions())
